...
(function () {
  ...
  theme.Variants = (function () {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function () {
        ...
        this._updateImagesCustom(variant);
      ...
      _updateImagesCustom: function (variant) {
        function handleUpdateGallery2() {
          ...
          const container = document.querySelector(`.product-main-slider`);
          if (template) {
            // the array is defined and has at least one element
            container.innerHTML = template.cloneNode(true).innerHTML;
  ...
  theme.CartForm = (function () {
    ...
    function CartForm(form) {
      ...
      this.subtotalNew = form.querySelector(selectors.subTotalNew);
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      updateSubtotal: function (subtotal) {
        ...
        this.form.querySelector(selectors.subTotalNew).innerHTML =
          theme.Currency.formatMoney(subtotal, theme.settings.moneyFormat);
      },
  ...
  const selectors = {
    wrapper: ".slide-nav__wrapper", // Vùng bao menu trượt
    nav: ".slide-nav", // Menu trượt
    childList: ".slide-nav__dropdown", // Danh sách con trong menu
    allLinks: "a.slide-nav__link", // Tất cả các liên kết trong menu
    subNavToggleBtn: ".js-toggle-submenu", // Nút mở submenu
    openBtn: ".mobile-nav-trigger", // Nút mở menu di động
  };

  const classes = {
    isActive: "is-active", // Lớp CSS để đánh dấu khi menu đang hoạt động
  };

  const defaults = {
    isOpen: false, // Menu mặc định là đóng
    menuLevel: 1, // Cấp độ của menu (1 là menu chính, 2 là submenu)
    inHeader: false, // Kiểm tra nếu menu nằm trong header
  };

  function MobileNav(args) {
    this.config = Object.assign({}, defaults, args); // Cấu hình với các tham số truyền vào
    this.namespace = ".nav-header-" + args.id; // Không gian tên cho sự kiện

    this.container = document.getElementById(this.config.id); // Lấy phần tử chứa menu
    if (!this.container) return;

    this.wrapper = this.container.querySelector(selectors.wrapper); // Lấy vùng bao menu
    if (!this.wrapper) return;

    this.nav = this.wrapper.querySelector(selectors.nav); // Lấy menu
    this.openTriggers = Array.from(document.querySelectorAll(selectors.openBtn)); // Lấy các nút mở menu
  ...
  MobileNav.prototype = {
    init: function () {
      ...
      if (this.openTriggers.length) {
        this.openTriggers.forEach((btn) => {
          btn.addEventListener("click", this.toggleNav.bind(this), { passive: true });
        });
      }

      this.nav.querySelectorAll(selectors.subNavToggleBtn).forEach((btn) => {
        btn.addEventListener("click", this.toggleSubNav.bind(this), { passive: true });
      });

      this.nav.querySelectorAll(selectors.allLinks).forEach((link) => {
        link.addEventListener("click", this.close.bind(this), { passive: true });
      });

      if (this.config.inHeader) {
        document.addEventListener("unmatchSmall", this.close.bind(this, null, true), { passive: true });
    ...
    toggleNav: function () {
      // Chuyển trạng thái mở/đóng menu
    ...
    open: function (evt) {
      ...
      theme.sizeDrawer(); // Điều chỉnh kích thước ngăn kéo

      this.openTriggers.forEach((btn) => btn.classList.add(classes.isActive));

      theme.utils.prepareTransition(this.container, () => {
        this.container.classList.add(classes.isActive);
      });

      window.addEventListener("keyup" + this.namespace, this.handleEsc.bind(this));
      ...
      setTimeout(() => {
        window.addEventListener("click" + this.namespace, this.close.bind(this), { passive: true });
      }, 0);
    },

    close: function (evt, noAnimate) {
      if (evt?.target.closest && evt.target.closest(".site-header__drawer")) {
        if (!evt.currentTarget.classList.contains("slide-nav__link")) return;
      }

      this.openTriggers.forEach((btn) => btn.classList.remove(classes.isActive));

      if (noAnimate) {
        this.container.classList.remove(classes.isActive);
      } else {
        theme.utils.prepareTransition(this.container, () => {
          this.container.classList.remove(classes.isActive);
        });
      ...
      window.removeEventListener("keyup" + this.namespace);
      window.removeEventListener("click" + this.namespace);

      this.config.isOpen = false;
    },

    handleEsc: function (evt) {
    ...
    goToSubnav: function (target) {
      const targetMenu = this.nav.querySelector(selectors.childList + `[data-parent="${target}"]`);
      if (targetMenu) {
