...
$('form.product-single__form').on('click', '.addon_addtocart', function(e) {
  ...
  fetch('/cart/add.js', {
    ...
    console.log(res);
    if (res.status && res.status == 422) {} else {
...
$(document).ready(function() {
  ...
  $(".variant-input-wrap[name='Furniture']").each(function() {
    let lessString = theme.strings.viewLess;
    let moreString = theme.strings.viewMore;
    var $mainDiv = $(this),
      $childrens = $mainDiv.children(),
      $a = $(`<a href='javascript:void(0)'>${lessString}</a>`)
